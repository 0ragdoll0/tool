...
function createVue(name) {
  const vueTemplate = templateDir + '/' + 'componentName.vue'
...
function createScss(name) {
  const styleTemplate = templateDir + '/' + 'componentName.comp.scss'
  fs.readFile(styleTemplate, function (error, data) {
    ...
    // 根据模板内容填充组件内容
    const styleFileName = name + '.comp.scss'
